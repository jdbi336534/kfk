...
void ICACHE_FLASH_ATTR 
gpio_set_input(uint32 gpio_pin, uint8 gpio_num)
{
	uint32 io_reg;
	
	GPIO_AS_INPUT(gpio_pin);
...
void ICACHE_FLASH_ATTR 
gpio_set_output(uint32 gpio_pin, uint8 gpio_num)
{
	uint32 io_reg;
	
	GPIO_AS_OUTPUT(gpio_pin);
	io_reg = GPIO_PIN_REG(gpio_num);
	PIN_PULLUP_EN(io_reg);
	PIN_FUNC_SELECT(io_reg, 0);
	gpio_pin_intr_state_set(gpio_num, GPIO_PIN_INTR_DISABLE);
...
uint8 ICACHE_FLASH_ATTR 
gpio_get_value(uint8 gpio_num)
{
	return (uint8)GPIO_INPUT_GET(GPIO_ID_PIN(gpio_num));
}
